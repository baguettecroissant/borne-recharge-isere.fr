#!/usr/bin/env node
/**
 * Verify content uniqueness across all local pages in Isère (38).
...
for (const category of CATEGORIES) {
  ...
  for (const c of communes) {
    const combo = [
      ...
      c.distanceGrenoble <= 5 ? 'core' : c.distanceGrenoble <= 15 ? 'inner' : c.distanceGrenoble <= 35 ? 'mid' : 'outer',
...
for (const c of communes) {
  ...
  distanceSet.add(c.distanceGrenoble);
